...
I found some summary information listing basic information about all of the publicly traded companies in US markets.  I then wrote script to collect and organize the company data into market sectors and then periodically refresh the high level data into a local spreadsheet.  From that data I chose the largest 100 companies from each market sector to keep a database of price and trading volume history, over 2,000 companies.  I wrote another script to update a local database with recent price and trading volume data.  The scrip will look to see if that stock is new to the database and if it is new then it will collect the past 10 years of data, otherwise it will simply update the local database with new data.

Once this data was properly organized and stored locally I can perform measurements for each stock.  First, I calculated the 6 month and 2 month moving averages of each stock's closing price and then identified each time that the 2 month moving average crosses higher than the 6 month moving average and each time that it crosses lower than the 6 month moving average.  Finally I found the last time there was a crossing of moving averages and defined this point as the start of the current streak.  

So now I calculated each stock's streak length in days and defined a regression line through the stock price during this streak to identify the stock price's upward or downward trend.  I then used the relationship between the moving averages to determine if the price was accelerating or decelerating.  For instance in the chart below we see there is an increasing trend in stock price ( rising grey trend line ) and we see by the way that the gap between the green 2 month moving average and red 6 month moving average lines is widening that price is accelerating upward.

<p align="center">  
    <img src="https://michaeljmerritt.github.io/Portfolio/Images/Stocks_Definition_a.jpg">
</p>
...
<p align="center">  
    <img src="https://michaeljmerritt.github.io/Portfolio/Images/Stocks_Definition_b.jpg">
</p>
